...
const Modal = ({ isOpen, setIsOpen, data }: props) => {
  return (
    <Dialog
      open={isOpen}
      onClose={setIsOpen}
      className="fixed inset-0 overflow-y-auto pt-[50vh]  sm:pt-[70vh]"
    >
      <Dialog.Overlay
        as={motion.div}
        variants={overlay}
      ...
      <Dialog.Panel
        ...
        exit="hidden"
        animate="animate"
        className="relative flex h-full w-full flex-col gap-3 rounded-t-3xl bg-emerald-800 p-5 md:h-screen md:w-1/4 md:rounded-none md:pt-[20%]"
      >
        ...
        <button
          className="focus: rounded-md bg-green-500 py-2 text-xl font-bold text-white focus:ring focus:ring-green-500 focus:ring-offset-2"
